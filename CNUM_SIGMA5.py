# -*- coding: utf-8 -*-
# =============================================================================
#                    AGENDA SIGMA ET JOURNAL DE MODIFICATIONS
# =============================================================================
...
def read_config(config_file="config.txt"):
    ...
    contenant tous les chemins et paramètres.
    """
    config = {}
...
# Utilisation des paramètres issus du fichier config.
...
# =============================================================================
# PARTIE 2 : SURVEILLANCE DES MODIFICATIONS
# =============================================================================
...
# Dictionnaire des couleurs -> lieux pour la surveillance
COLOR_TO_LOCATION = {
...
def charger_etat_excel():
    """
    Charge l'état actuel des cellules du fichier Excel en prenant en compte
    les cellules fusionnées sur la feuille spécifiée.
    """
...
def format_cell_data(cell_data):
    """Formate les données de la cellule pour l'affichage dans le CSV."""
    parts = []
...
def comparer_etats(etat_precedent, etat_actuel):
    """Compare deux états et retourne les modifications détectées."""
    modifications = []
...
def enregistrer_modifications(modifications):
    """Enregistre les modifications détectées dans le CSV de modifications."""
    ...
    with open(CSV_MODIFICATIONS, mode="a", newline="", encoding="utf-8") as file:
        ...
        if not fichier_existe:
            writer.writerow(["Date", "Heure", "Cellule", "Feuille", "Ancienne Donnée", "Nouvelle Donnée"])
...
def surveiller_excel():
    """Boucle infinie surveillant les modifications dans le fichier Excel."""
...
# =============================================================================
# PARTIE 3 : GÉNÉRATION DU CSV POUR L'AGENDA
# =============================================================================

# Constantes pour l'extraction des événements
horaires = {
...
def split_subject_into_events(subject, date_str, halfday_label, location, description):
    """
    Retourne une liste d'événements [Subject, Date, Start Time, End Time, Location, Description].
    """
    events = []
...
def process_agenda():
    """
    Traite la feuille indiquée pour générer un CSV unique pour l'agenda,
    puis lance la synchronisation avec Google Calendar.
    """
    events_global = []
...
def sync_events(service, df):
    ...
    Le csv_id est généré à partir de Date, Start Time et Location.
    """
    room_colors = {
    ...
    for _, row in df.iterrows():
        # Génération stable du csv_id : Date, Start Time et Location
        raw_id = f"{row['Date']}_{row['Start Time']}_{row['Location']}"
        ...
        color_id = room_colors.get(location, 5)
        event_body = {
            'summary': row['Subject'],
            'location': location,
    ...
    new_csv_ids = set(sanitize_csv_id(f"{row['Date']}_{row['Start Time']}_{row['Location']}") for _, row in df.iterrows())
...
def run_modifications():
    """Lance la surveillance des modifications."""
    surveiller_excel()

def run_agenda():
    """
    Exécute périodiquement la génération du CSV pour l'agenda
    et la synchronisation avec Google Calendar (toutes les 5 minutes).
    """
    while True:
        try:
            process_agenda()
